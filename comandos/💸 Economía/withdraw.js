...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (["todo", "all-in", "all"].includes(args[0])) {
      ...
      if (isNaN(cantidad) || cantidad <= 0 || cantidad % 1 != 0)
        return message.reply(
          "❌ **No has especificado una cantidad válida para sacar!**"
        );
      if (cantidad > data.banco)
        return message.reply("❌ **No tienes tanto dinero para sacar!**");
    }
    ...
    return message.reply(
      `✅ **Has sacado \`${cantidad} monedas\` de tu banco!**`
    );
  },
};
