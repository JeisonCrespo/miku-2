...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (usuario.id == message.guild.ownerId)
      return message.reply(`❌ **No puedes expulsar al DUEÑO del Servidor!**`);
    ...
    if (
      ...
      if (
        ...
        message.reply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setDescription(
                `**Se ha expulsado exitosamente a \`${usuario.user.tag}\` *(\`${usuario.id}\`)* del servidor!**`
              )
              .addFields([
                { name: `Razón`, value: `\n\`\`\`yml\n${razon}\`\`\`` },
              ])
        ...
        return message.reply(
          `❌ **Tu Rol está por __debajo__ del usuario que quieres banear!**`
        );
      }
    } else {
      return message.reply(
        `❌ **Mi Rol está por __debajo__ del usuario que quieres banear!**`
      );
    }
  },
};
