  ...
  run: async (client, message, args, prefix) => {
    ...
    async function emoji() {
      ...
      let preguntar = await message.reply({
        embeds: [
          new Discord.EmbedBuilder()
            ...
            .setDescription(
              `*Reacciona en **__este mensaje__** con el emoji deseado!*\n\nEscribe \`finalizar\` para terminar el setup!`
            )
      ...
      preguntar.channel
        ...
        .then((collected) => {
          if (
            collected.first().content.toLowerCase() === "finalizar" &&
            !finalizado
          ) {
            finalizado = true;
            return finalizar();
          ...
          .then(async (collected) => {
            ...
            if (rol) {
              ...
              return message.reply(
                `❌ **El Rol que has mencionado NO EXISTE!**\nSetup cancelado!`
              );
            }
          })
          .catch(() => {
    ...
    async function finalizar() {
      if (contador === 1 && !objeto.Parametros.length)
        return message.reply(
          `❌ **Tienes que añadir al menos un emoji con un rol**\nSetup cancelado!`
        );
      let msg = await message.reply({
        embeds: [
          new Discord.EmbedBuilder()
            ...
            .setDescription(
              `⬇ Menciona o escribe la ID del canal del mensaje a reaccionar! ⬇`
            )
            .setColor(client.color),
        ],
      });
      await msg.channel
        ...
        .then(async (collected) => {
          ...
          if (canal) {
            ...
            var idmensaje = await message.reply({
              embeds: [
                new Discord.EmbedBuilder()
                  ...
                  .setDescription(
                    `⬇ Escribe la ID del mensaje para añadir las reacciones! ⬇`
                  )
                  .setColor(client.color),
              ],
            });
            await idmensaje.channel
              .awaitMessages({
                filter: (m) => m.author.id === message.author.id,
                max: 1,
                errors: ["time"],
              ...
              .then(async (collected) => {
                var message = collected.first();
                const encontrado = await message.guild.channels.cache
                  .get(objeto.ID_CANAL)
                  .messages.fetch(message.content);
                if (encontrado) {
                  for (var i = 0; i < objeto.Parametros.length; i++) {
                    encontrado.react(objeto.Parametros[i].Emoji).catch(() => {
                      console.log("NO SE HA PODIDO AÑADIR LA REACCIÓN");
                    });
                  }
                  objeto.ID_MENSAJE = message.content;
                  let setupdatos = await setupSchema.findOne({
                    guildID: message.guild.id,
                  });
                  setupdatos.reaccion_roles.push(objeto);
                  setupdatos.save();
                  return message.reply(
                    `Setup de Reacción con Roles Creado ✅\nPuedes crear otro setup ejecutando el comando \`${prefix}setup-reactionroles\``
                  );
                } else {
                  return message.reply(
                    `❌ **No se ha encontrado el mensaje que has especificado!**\nSetup cancelado!`
                  );
                }
              })
              .catch((e) => {
            ...
            return message.reply(
              `❌ **No se ha encontrado el canal que has especificado!**\nSetup cancelado!`
            );
          }
        })
        .catch((e) => {
