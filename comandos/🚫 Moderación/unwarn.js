  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!usuario)
      return message.reply(
        `❌ **No se ha encontrado al usuario que has especificado!**`
      );
    ...
    if (data.warnings.length === 0)
      return message.reply(
        `❌ **El usuario que has especificado no tiene ningún warning!**`
      );
    if (!id_warn)
      return message.reply(
        `❌ **No se ha encontrado el warn que has especificado!**`
      );
    if (isNaN(id_warn) || id_warn < 0)
      message.reply(`❌ **La ID del warn que has especificado no es válida!**`);
    ...
    if (usuario.id == message.guild.ownerId)
      return message.reply(`❌ **No puedes avisar al DUEÑO del Servidor!**`);
    ...
    if (
      ...
      if (
        message.member.roles.highest.position > usuario.roles.highest.position
      ) {
        message.reply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setDescription(
                `**Se ha removido el warn con ID \`${id_warn}\` de \`${usuario.user.tag}\` exitosamente!**`
              )
        ...
        return message.reply(
          `❌ **Tu Rol está por __debajo__ del usuario que quieres avisar!**`
        );
      }
    } else {
      return message.reply(
        `❌ **Mi Rol está por __debajo__ del usuario que quieres avisar!**`
      );
    }
  },
};
