  ...
  run: async (client, message, args, prefix) => {
    ...
    for (let i = 0; i < queue.songs.length; i += maximascanciones) {
      ...
      listaqueue.push(
        canciones
          .map(
            (cancion, index) =>
              `**\`${i + ++index}\`** - [\`${cancion.name}\`](${cancion.url})`
          )
          .join("\n ")
      );
    }
    ...
    for (let i = 0; i < limite; i++) {
      ...
      let embed = new Discord.EmbedBuilder()
        .setTitle(
          `🎶 Cola de ${message.guild.name} - \`[${queue.songs.length} ${
    ...
    async function paginacion() {
      ...
      let boton_atras = new Discord.ButtonBuilder()
        .setStyle("Success")
        .setCustomId("Atrás")
        .setEmoji("929001012176507040")
        .setLabel("Atrás");
      let boton_inicio = new Discord.ButtonBuilder()
        .setStyle("Danger")
        .setCustomId("Inicio")
        .setEmoji("🏠")
        .setLabel("Inicio");
      let boton_avanzar = new Discord.ButtonBuilder()
        .setStyle("Success")
        .setCustomId("Avanzar")
      ...
      collector.on("collect", async (b) => {
        ...
        if (b?.user.id !== message.author.id)
          return b?.reply({
            content: `❌ **Solo la persona que ha escrito \`${prefix}queue\` puede cambiar de páginas!`,
          });
      ...
      collector.on("end", () => {
        ...
        embedpaginas.components[0].components.map(
          (boton) => (boton.disabled = true)
        );
        embedpaginas
          .edit({
            content: `El tiempo ha expirado! escribe de nuevo \`${prefix}queue para volver a ver la cola de canciones!\``,
            embeds: [
              embeds[paginaActual].setFooter({
                text: `Pagina ${paginaActual + 1} / ${embeds.length}`,
              }),
