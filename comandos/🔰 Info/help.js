    ...
    if (args[0]) {
      ...
      if (comando) {
        let embed = new Discord.EmbedBuilder()
          ...
          .setFooter({
            text: `© desarrollado por dewstouh#1088 | 2022`,
            iconURL: `https://cdn.discordapp.com/avatars/282942681980862474/7ff4f4ae92af5feb0d258a71cdb0b060.png?size=4096`,
          })
          .setColor(client.color);
        //condicionales
        if (comando.desc)
          embed.addFields([
            { name: `✍ Descripción`, value: `\`\`\`${comando.desc}\`\`\`` },
          ]);
        ...
        if (comando.permisos && comando.permisos.length >= 1)
          embed.addFields([
            {
              name: `👤 Permisos requeridos`,
              value: `${comando.permisos
                .map((permiso) => `\`${permiso}\``)
                .join(", ")}`,
            },
          ]);
        if (comando.permisos_bot && comando.permisos_bot.length >= 1)
          embed.addFields([
            {
              name: `🤖 Permisos de BOT requeridos`,
              value: `${comando.permisos_bot
                .map((permiso) => `\`${permiso}\``)
                .join(", ")}`,
            },
          ]);
        ...
        const comandos_de_categoria = readdirSync(
          `./comandos/${categoria}`
        ...
        return message.reply({
          embeds: [
            new Discord.EmbedBuilder()
              .setTitle(
                `${categoria.split(" ")[0]} ${categoria.split(" ")[1]} ${
                  categoria.split(" ")[0]
                }`
              )
              .setColor(client.color)
              .setDescription(
                comandos_de_categoria.length >= 1
                  ? `>>> *${comandos_de_categoria
                      .map((comando) => `\`${comando.replace(/.js/, "")}\``)
                      .join(" - ")}*`
        ...
        return message.reply(
          `❌ **No se ha encontrado el comando que has especificado!**\nUsa \`${prefix}help\` para ver los comandos y categorías!`
        );
      }
    } else {
      var paginaActual = 0;
      ...
      let ayuda_embed = new Discord.EmbedBuilder()
        ...
        .setColor(client.color)
        .setDescription(`Bot Multifuncional en desarrollo por \`Jeison\``)
        .addFields([
          {
            name: `❓ **__¿Quién soy?__**`,
            value: `👋 Hola **${message.author.username}**, mi nombre es **__${client.user.username}__**\n🤯 Soy un BOT MULTIFUNCIONAL Incluyendo:\n> **ADMINISTRACIÓN\n> MODERACIÓN\n> MÚSICA**\n*y mucho más!*`,
          },
        ])
        .addFields([
          {
            name: `📈 **__ESTADÍSTICAS__**`,
            value: `⚙ **${client.commands.size} Comandos**\n📁 en **${client.guilds.cache.size} Servidores**\n📶 **\`${client.ws.ping}ms\` Ping**\n👤 desarrollado por **[Jeison](https://discord.gg/NqsabHxXdY)**`,
          },
        ])
        ...
        .setFooter({
          text: `Página 1 / ${
            categorias.length + 1
          }\n© desarrollado por Jeison | 2022`,
          iconURL: `https://cdn.discordapp.com/avatars/256114822083510283/cdb62a43d87858ab76951b4d498eb1d5.png?size=4096`,
        });
      ...
      const seleccion = new Discord.ActionRowBuilder().addComponents(
          ...
          .addOptions(
            categorias.map((categoria) => {
              ...
              let objeto = {
                ...
                value: categoria,
                description: `Mira los comandos de ${categoria
                  .split(" ")[1]
                  .substring(0, 50)}`,
      ...
      let mensaje_ayuda = await message.reply({
        embeds: [ayuda_embed],
      ...
      collector.on("collect", async (interaccion) => {
        if (interaccion.isButton()) {
          if (interaccion.user.id !== message.author.id)
            return interaccion.reply({
              content: `❌ **No puedes hacer eso! Solo ${message.author}**`,
              ephemeral: true,
            });
          ...
          for (const seleccionado of interaccion.values) {
            ...
            let embed = new Discord.EmbedBuilder()
              .setTitle(
                `${seleccionado.split(" ")[0]} ${seleccionado.split(" ")[1]} ${
              ...
              .setFooter({
                text: `© desarrollado por Jeison | 2022`,
                iconURL: `https://cdn.discordapp.com/avatars/282942681980862474/7ff4f4ae92af5feb0d258a71cdb0b060.png?size=4096`,
              });
      ...
      collector.on("end", () => {
        mensaje_ayuda
          .edit({
            content: `Tu tiempo ha expirado! Vuelve a escribir \`${prefix}help\` para verlo de nuevo!`,
            components: [],
          })
          .catch(() => {});
      });
