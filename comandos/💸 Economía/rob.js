  ...
  run: async (client, message, args, prefix) => {
    if (!args.length)
      return message.reply(
        "❌ **Tienes que especificar al usuario para robar!**"
      );
    const usuario =
    ...
    if (tiempo_ms - (Date.now() - data.rob) > 0) {
      ...
      return message.reply(
        `🕑 **Tienes que esperar \`${tiempo_restante}\` para volver a robar a un usuario!**`
      );
    }
    let data_usuario = await ecoSchema.findOne({ userID: usuario.id });
    if (data_usuario.dinero < 500)
      return message.reply(
        "❌ **No puedes robar al usuario ya que, tiene menos de `500 monedas`**"
      );
    let cantidad = Math.floor(Math.random() * 400) + 100;
    //comprobaciones previas
    if (cantidad > data_usuario.dinero)
      return message.reply(
        "❌ **El usuario no tiene suficiente dinero para ser robado!**"
      );
    ...
    return message.reply(
      `✅ **Has robado \`${cantidad} monedas\` a \`${usuario.user.tag}\`**`
    );
  },
};
