...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (["todo", "all-in", "all"].includes(args[0])) {
      ...
      if (isNaN(cantidad) || cantidad <= 0 || cantidad % 1 != 0)
        return message.reply(
          "❌ **No has especificado una cantidad válida para depositar!**"
        );
      if (cantidad > data.dinero)
        return message.reply("❌ **No tienes tanto dinero para depositar!**");
    }
    ...
    return message.reply(
      `✅ **Has depositado \`${cantidad} monedas\` en tu banco!**`
    );
  },
};
