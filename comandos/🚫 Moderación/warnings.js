  ...
  run: async (client, message, args, prefix) => {
    ...
    if (data.warnings.length == 0)
      return message.reply(
        `✅ **\`${usuario.user.tag}\` no tiene ningún warning en el servidor!**`
      );
    const texto = data.warnings.map(
      (warn, index) =>
        `================================\n**ID DE WARN:** \`${index}\`\n**FECHA:** <t:${Math.round(
          ...
          message.guild.members.cache.get(warn.autor).user.tag
        }\`*\n**RAZÓN:** \`${warn.razon}\`\n`
    );
    paginacion(
      client,
      message,
      texto,
      `🛠 \`[${data.warnings.length}]\` WARNINGS DE ${usuario.user.tag}`,
      1
    );
  },
};
