  ...
  run: async (client, message, args, prefix) => {
    ...
    if (!canalNotificaciones)
      return message.reply(
        "❌ **No has especificado un canal de notificaciones al subir de nivel!**"
      );

    const mensaje = args.slice(1).join(" ").substring(0, 2048);
    if (!mensaje)
      return message.reply(
        "❌ **No has especificado un mensaje cuando un usuario sube de nivel!**"
      );
    ...
    return message.reply({
      embeds: [
        new Discord.EmbedBuilder()
          ...
          .setDescription(
            `*Enviaré las notificaciones cuando un usuario suba de nivel en ${canalNotificaciones}*`
          )
          .setColor("Green"),
      ],
    });
