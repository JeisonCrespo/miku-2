...
var trabajos = ["Camionero", "desarrollador", "Mecánico", "Taxista"];
module.exports = {
  name: "work",
  aliases: ["trabajar"],
  ...
  run: async (client, message, args, prefix) => {
    ...
    if (tiempo_ms - (Date.now() - data.work) > 0) {
      ...
      return message.reply(
        `🕑 **Tienes que esperar \`${tiempo_restante}\` para volver a reclamar tu recompensa diaria!**`
      );
    }
    ...
    return message.reply(
      `✅ **Has trabajado como \`${trabajo}\` y has recibido una recompensa de \`${recompensa} monedas\`!**`
    );
  },
};
