...
module.exports = (client) => {
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    try {
      ...
      if (
        !reaccionroles ||
        !reaccionroles.length ||
        reaccionroles.length === 0 ||
        reaccionroles === undefined ||
        reaccionroles === null
      )
        return;
            ...
            if (reaction.emoji?.id == parametros[k].Emoji) {
              try {
                ...
                if (role) {
                  ...
                  if (
                    usuario.guild.members.me.roles.highest.rawPosition >
                    role.position
                  ) {
                    await usuario.roles.add(rol).catch((e) => {
                      e;
                    });
                    ...
                    reaction.message.channel
                      .send(
                        `❌ **No tengo suficientes permisos para dar el rol!**\nAsegúrate de mi rol esté por encima del rol a dar!`
                      )
                      .catch(() => {})
                      .then((msg) => {
                        setTimeout(() => {
                  ...
                  reaction.message.channel
                    .send(`❌ **Ese rol ha sido eliminado!**`)
                    .catch(() => {})
                    .then((msg) => {
                      setTimeout(() => {
                        msg.delete();
              ...
              try {
                ...
                if (role) {
                  //Si el rol a dar está por debajo de los roles del bot, le damos el rol
                  if (
                    usuario.guild.members.me.roles.highest.position >
                    role.position
                  ) {
                    await usuario.roles.add(rol).catch((e) => {
                      e;
              ...
              try {
                ...
                if (role) {
                  ...
                  if (
                    ...
                    if (usuario.roles.cache.has(rol))
                      await usuario.roles.remove(rol).catch(() => {});
                  } else {
                    //Si no, entonces hacemos return respondiendo que el bot no tiene permisos
                    reaction.message.channel
                      .send(
