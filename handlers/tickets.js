...
module.exports = (client) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      if (
        ...
        interaction.customId !== "crear_ticket"
      )
        return;
      ...
      const setup = await setupSchema.findOne({
        guildID: interaction.guild.id,
      });
      //comprobaciones previas
      if (
        !setup ||
        !setup.sistema_tickets ||
        !setup.sistema_tickets.canal ||
        interaction.channelId !== setup.sistema_tickets.canal ||
        interaction.message.id !== setup.sistema_tickets.mensaje
      )
        return;
      //buscamos primero si el usuario tiene un ticket creado
      let ticket_data = await ticketSchema.find({
        guildID: interaction.guild.id,
      ...
      for (const ticket of ticket_data) {
        if (interaction.guild.channels.cache.get(ticket.canal))
          return interaction.reply({
            content: `❌ **Ya tienes un ticket creado en <#${ticket.canal}>**`,
            ephemeral: true,
          });
      }
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      let ticket_data = await ticketSchema.findOne({
        guildID: interaction.guild.id,
        ...
        case "cerrar_ticket":
          {
            ...
            collector.on("collect", (boton) => {
              ...
              if (boton.user.id !== interaction.user.id)
                return boton.reply({
                  content: `❌ **No puedes hacer eso! Solo ${interaction.user} puede!**`,
                  ephemeral: true,
                });
              ...
              interaction.channel.permissionOverwrites.edit(ticket_data.autor, {
                ViewChannel: false,
              });
              interaction.channel.send(
                `✅ **Ticket Cerrado por \`${
                  interaction.user.tag
                }\` el <t:${Math.round(Date.now() / 1000)}>**`
              );
            });
        ...
        case "borrar_ticket":
          {
            ...
            collector.on("collect", (boton) => {
              ...
              ticket_data.delete();
              interaction.channel.send(
                `✅ **El ticket será eliminado en menos de \`3 segundos ...\`\nAcción por: \`${
                  interaction.user.tag
                }\` el <t:${Math.round(Date.now() / 1000)}>**`
              );
